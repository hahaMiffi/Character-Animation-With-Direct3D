...
bool KeyUp( int vk_code ){ return (::GetAsyncKeyState(vk_code) & 0x8000) ? true : false;  }
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE, LPSTR cmd, int )
{
	...
	while( msg.message != WM_QUIT )
	{
		...
		else
		{
			...
			float deltaTime = ( t - startTime ) * 0.001;
...
HRESULT Application::Init( HINSTANCE hInstance, bool windowed )
{
	...
	m_mainWindow = CreateWindow( "D3DWND", "Example1", WS_OVERLAPPEDWINDOW, 0, 0, rc.right-rc.left, rc.bottom - rc.top,
								0, 0, hInstance, 0 );
	SetCursor( NULL );
	...
	D3DXCreateFont( g_pDevice, 20, 0, FW_BOLD, 1, false, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, DEFAULT_QUALITY, 
					DEFAULT_PITCH |	FF_DONTCARE, "Arial", &g_pFont );

	D3DXCreateSprite( g_pDevice, &g_pSprite );

	ID3DXBuffer* pErrorMsgs = NULL;
	HRESULT hRes = D3DXCreateEffectFromFile( g_pDevice, "resources/fx/lighting.fx", 0, 0, D3DXSHADER_DEBUG, 0, &g_pEffect, &pErrorMsgs );
...
void Application::Render()
{
	if( !m_deviceLost )
	{
		try
		{
			...
			D3DXMatrixLookAtLH( &view, &D3DXVECTOR3(cos(m_angle) * 2.0f, 2.0f, sin(m_angle) * 2.0f), &D3DXVECTOR3( 0.0f, 1.0f, 0.0f ),
